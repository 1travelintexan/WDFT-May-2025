...
function multiplyBy(arrayOfNums, mult) {
  const newArray = [];
  arrayOfNums.forEach((oneNumber) => {
    const newNumber = oneNumber * mult;
...
function greatestProduct(matrix) {
  ...
  for (let i = 0; i < matrix.length - 3; i++) {
    ...
    for (let j = 0; j < row.length - 3; j++) {
      const number = row[j];
      const subTotal = number * row[j + 1] * row[j + 2] * row[j + 3];
      if (subTotal > total) {
        total = subTotal;
      }
      //check vertically for the subtotal
      const verticalNumber =
        matrix[i][j] * matrix[i + 1][j] * matrix[i + 2][j] * matrix[i + 3][j];
      if (verticalNumber > total) {
        total = verticalNumber;
      }
    }
  }
  return total;
