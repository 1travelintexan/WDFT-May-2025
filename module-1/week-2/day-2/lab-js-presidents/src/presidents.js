...
function getNames(presidentsArr) {
  const namesArray = presidentsArr.map((onePres) => {
    return onePres.name;
  });
...
function getDemocraticPresidents(presidentsArr) {
  const demsPresArr = presidentsArr.filter((pres) => {
    if (pres.party === "Democratic") {
      return pres.party;
    }
  });
  return demsPresArr;
  ...
  //oneliner version
  // return presidentsArr.filter((pres) => pres.party === "Democratic");
}

// Iteration 3 | Count Years in Office - reduce()
function countYearsInOffice(presidentsArr) {
  const totalYears = presidentsArr.reduce((acc, currentPresident) => {
    if (currentPresident.leftOffice) {
      const onePresYearsInOffice =
        currentPresident.leftOffice - currentPresident.tookOffice;
      return acc + onePresYearsInOffice;
    } else {
      return acc;
    }
  }, 0);
...
function getAgeAtInauguration(presidentsArr) {
  const newPresArray = presidentsArr.map((onePres) => {
    const age = onePres.tookOffice - onePres.birthYear;
    onePres.ageAtInauguration = age;
    return onePres;
  });
  return newPresArray;
}
...
function getPresidentsBornAfter(presidentsArr, year) {
  const filteredPresArr = presidentsArr.filter((onePres) => {
    if (onePres.birthYear > year) {
      return onePres;
    }
  });
  return filteredPresArr;
}

// Bonus: Iteration 7 | Count Republican Presidents
function countRepublicanPresidents(presidentsArr) {
  const repPresCount = presidentsArr.reduce((acc, curr) => {
